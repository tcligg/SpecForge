...
- [x] **Step 5** — `gke/scheduling.py` pending-reason classifier (`PendingReason`, `PodDiagnosis`, `classify_pod`, `diagnose_job`, `summarize_pending`); `gke/lib/k8s.py` extended with `get_pods_json` + `get_pod_events`; wired behind `--enable-scheduling-v2` (default off, observation-only) in both `deploy.py` and `orchestrator.py`
- [x] **Step 6** — `deploy_dataset_strict` in `gke/lib/orchestration.py` (serial fall-through, classifier-driven decisions, sleep-retry on spot exhaustion); orchestrator's Phase C uses it + does adopt-existing-job from state; new tunables `--attempt-deadline`, `--monitor-deadline`, `--spot-exhausted-strategy`, `--spot-retry-interval`, `--max-wait-hours`; `min_running = total_pods` strict default in the orchestrator path; `gke/deploy.py --execute` carries a `DeprecationWarning`
- [x] **Step 7** — `gke/rescue.py` with `compute_missing_chunks`, `_write_rescue_yaml`, `phase_d5_rescue_dataset`; orchestrator wires D.5 between D and E (triggers when chunks are missing OR D failed); new `rescue` subcommand for manual trigger; new tunables `--max-rescue-attempts`, `--max-rescue-shards`; uses existing `EXTRA_REGEN_ARGS` env hook (no `regen_entrypoint.py` change needed)
- [x] **Step 8** — deleted `gke/deploy.sh` and `gke/regen_entrypoint.sh`; reduced `gke/deploy.py` from 448 to 221 lines (`--status` / `--delete` kept; `--execute` redirects to the orchestrator); README updated; YAMLs already in sync from step 4b
...
| 2026-04-22 | Step 5: classifier is a pure function of `(pod_dict, events, now)` | Lets unit tests drop in captured kubectl JSON fixtures without subprocess plumbing. The thin `diagnose_job` orchestrator does the kubectl shell-outs in one place |
| 2026-04-22 | Step 5: kubectl `-o json` over `kubectl describe` | Structured access to `.status.conditions` and `.status.containerStatuses[*].state.waiting.reason`; no regex over human text. Fixture-friendly |
| 2026-04-22 | Step 5: config-error patterns checked **before** capacity patterns | Real Unschedulable messages have the form `0/N nodes are available: <reason>`. PVC/secret/affinity/toleration errors are operator-fixable and must short-circuit before the `0/N nodes available` prefix gets attributed to capacity exhaustion |
| 2026-04-22 | Step 5: image-pull errors take precedence over scheduling-message classification | A waiting `ImagePullBackOff` reason always means bail; even if the pod also has a scheduling message attached (because subsequent containers depend on the image), the operator response is the same |
| 2026-04-22 | Step 5: scale-up event with missing timestamp treated as fresh (CAPACITY_WAIT) | Defensive — if kubectl drops the timestamps for some reason, waiting one extra cycle is cheaper than a false-positive CAPACITY_EXHAUSTED that ejects a workload that's actually about to schedule |
| 2026-04-22 | Step 5: deadline half of CAPACITY_EXHAUSTED ("deadline passed") deferred to step 6 | Step 5 returns CAPACITY_EXHAUSTED whenever GPU-blocked + no fresh scale-up. Step 6 owns the `attempt_deadline` (10 min default) that combines with this signal to decide when to fall over to the next candidate |
| 2026-04-22 | Step 5: `--enable-scheduling-v2` is observation-only; legacy `min_running` still drives the wait loop | Lets us validate classifier output against real production GKE behavior before step 6 makes scheduling decisions on it. Lazy-imports `gke.scheduling` so the classifier's dependencies don't load when the flag is off |
| 2026-04-22 | Step 6: `deploy_dataset_strict` lives next to `deploy_dataset` in `gke/lib/orchestration.py` (not a new module) | Both algorithms cohere around the same lib helpers (`patch_and_apply_yaml`, `get_pod_status`, etc.); step 8 deletes the legacy one as a single-file edit. New module would mean two files holding similar concepts, more churn at cleanup |
| 2026-04-22 | Step 6: adopt-existing-job lives in the orchestrator, not the deploy lib | The orchestrator owns the state file. Keeping the deploy lib state-agnostic preserves the step-3 architecture (state I/O at the orchestrator boundary; lib helpers stay pure-ish over kubectl). Trade-off: the orchestrator's Phase C is a bit fatter; acceptable given the lib shrinks at step 8 anyway |
| 2026-04-22 | Step 6: candidate iteration order is GPU-first, then cluster | Operator passes GPU types in preference order (`--gpu-types h200,h100`); we honour that intent. Per-cluster historical success-rate ordering is a deferred improvement in PLAN.md "Open items" |
| 2026-04-22 | Step 6: CONFIG_ERROR / IMAGE_PULL_ERROR raise `StrictDeployError` and abort the whole pipeline, not just the dataset | Both errors are global by nature — the image is shared across datasets, and YAML config errors usually hit every dataset on the same template. Skipping to the next dataset would just hit the same error and waste the spot capacity we just acquired |
| 2026-04-22 | Step 6: every pending pod must classify as CAPACITY_EXHAUSTED to trigger fall-over before the deadline | If even one pod is still CAPACITY_WAIT (fresh scale-up) or UNKNOWN, the candidate keeps waiting. Avoids fast-failing a job that's mid-scale-up because one pod hit a transient pull error. Deadline still bounds the total wait |
| 2026-04-22 | Step 6: orchestrator's `--enable-scheduling-v2` flag becomes a no-op | Strict scheduling is the orchestrator's only path now; the flag remains parseable for back-compat but doesn't do anything in `run`. The `gke/deploy.py --execute` legacy path still consults it (for the classifier-logging behavior shipped in step 5) |
| 2026-04-22 | Step 6: `min_running` flipped to `total_pods` only inside `_build_deploy_config`, not in `Config.from_args` | Keeps `gke/deploy.py --execute` (the legacy path) on its old `ceil(N/2)` behavior so we don't change semantics for users who haven't migrated. `--execute` carries a `DeprecationWarning` so they know it's going away |
| 2026-04-22 | Step 7: rescue uses the existing `EXTRA_REGEN_ARGS` env hook in `regen_entrypoint.py`; no entrypoint change needed | The entrypoint already shlex-splits `EXTRA_REGEN_ARGS` onto the regen command line. PLAN.md:354-355 expected a fresh `CHUNK_IDS`-aware addition; the existing hook covers it cleanly. Step-1's `--chunk-ids` argparse was the only entrypoint-side requirement |
| 2026-04-22 | Step 7: rescue triggers on missing chunks even when Job reports Complete | The scheduler has been observed to mark Jobs Complete while a few shards quietly never produced output (spot preemption mid-finish, etc.). Probing `.done` markers is cheap; trusting only Job status would silently leak chunks |
| 2026-04-22 | Step 7: rescue `metadata.name` suffixed with `-rescue` (not `-rescue-attempt-N`) | The strict-scheduling helper appends `-<dataset>-<gpu>-<region>` per-attempt anyway; layering an attempt counter on top would produce unwieldy 80-char job names. Per-attempt distinction lives in state's `rescue.attempts[]` records, not in the job name |
| 2026-04-22 | Step 7: rescue YAML written to a temp sibling (`<src>.rescue.attemptN.yaml`) rather than mutating the primary YAML | Phase B already writes the resolved image into the primary YAML; mutating again for rescue would race with a possible Phase B re-run on resume. Sibling files are cheap and keep the primary YAML stable |
| 2026-04-22 | Step 7: rescue's missing-chunk math uses `state.prepare[ds].n_lines` over re-listing GCS | Phase A already counted lines and recorded them; re-listing the input file would add a second `gsutil cat | wc -l` round-trip. Trade-off: if the operator manually edited the prepared file in GCS between A and rescue, the math is wrong — but that's a rare manual escape hatch and the operator can re-run Phase A |
| 2026-04-22 | Step 7: rescue jobs raise `RescueError` (not abort the whole pipeline) on exhaustion | Different from step 6's `StrictDeployError`: rescue failure for one dataset shouldn't kill processing of the next (their merges are independent). The orchestrator catches `RescueError` and skips merge for that dataset, then continues |
| 2026-04-22 | Step 8: `--execute` becomes an `os.execvp` redirect rather than just a deprecation warning | Re-execing means legacy callers get the actual fixed pipeline (state-tracked, classifier-driven, rescue-aware) instead of the racing pattern. A warning-only path would silently keep them on the broken algorithm until the next round of cleanup |
| 2026-04-22 | Step 8: YAMLs were already synced (in step 4b at `pyc-vtx-dev/pyc-vtx-us-central1`, gemma34's registry) — not at PLAN.md's originally-named `cloud-llm-test/tcli-test` | The PLAN was written before the PR-fork pivot; what mattered was YAML ↔ build_and_push.sh ↔ orchestrator-defaults coherence, which we have. Cross-environment ports happen separately, not as part of the rebuild |
| 2026-04-22 | Step 8: AGENTS.md mentioned in PLAN doesn't exist in the repo; skipped | Aspirational reference. README + PLAN cover the design and operator guidance; a third doc would just diverge |
| 2026-04-23 | Step 9: SCALE_UP_FRESH_SECONDS widened from 5 min to 10 min after first e2e | The 5-min window misclassifies pre-empted-then-rescaling pods as CAPACITY_EXHAUSTED if their original `TriggeredScaleUp` event aged out before re-attempt. 10 min covers the typical autoscaler re-trigger latency. No-regret change: when the autoscaler explicitly refused (the more common case in our e2e), `TriggeredScaleUp` never existed, so widening the window doesn't change behavior |
| 2026-04-23 | Step 9: --attempt-deadline default widened from 600s to 1800s | Image pull on a fresh spot node for our 38GB sglang-based image takes several minutes; container init (sglang server warmup) eats more. 10 min budget bites slow-but-working candidates more than autoscaler-refused ones (the latter fall over within seconds via the "all pending pods CAPACITY_EXHAUSTED" rule, regardless of deadline). 30 min lets a working candidate complete without changing fast-fail behavior |
